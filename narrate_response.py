...
import os
from explain_tabular import explain_tabular
from explain_image import explain_image
from data_loader import load_car_sample  # or from data_loader_lab if you’re using lab samples


def generate_narrative(tabular_data=None, image_path=None):
    """
    Creates multimodal explanation with SHAP + GradCAM + narrative.
    """
    if tabular_data is None or image_path is None:
        tabular_data, image_path = load_car_sample()

    # Assert type
    assert isinstance(tabular_data, pd.DataFrame), "tabular_data must be a pandas DataFrame"


    # Generate SHAP plot
    shap_plot_path, shap_scores = explain_tabular(tabular_data)

    # Generate GradCAM plot
    gradcam_path = explain_image(image_path)

    # Create text explanations
    top_features = sorted(shap_scores.items(), key=lambda x: abs(x[1]), reverse=True)[:3]
    text_narr = "The model estimated the car's price by considering key factors such as " + \
        ", ".join([f"{feat} ({round(score, 2)})" for feat, score in top_features]) + "."

    non_tech_summary = (
        "Imagine explaining the car’s value like a mechanic checking both the logbook and the car’s photo. "
        "The model saw the year, mileage, and fuel type in the data, but also looked closely at visible features "
        "like paint or shape."
    )
...
if __name__ == "__main__":
    ...
    print("✅ Multimodal narrative generated.")
